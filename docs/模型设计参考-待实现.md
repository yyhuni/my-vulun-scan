...
| OriginalInput | string(100) | 可为空 | NULL | 原始输入：仅CIDR输入时使用 |
| InputType | string(20) | 非空 | domain | 输入类型：domain/ip/cidr |
| Description | text | 可为空 | NULL | 描述信息 |
...
| AttackSurface | text | 可为空 | NULL | 攻击面信息 |
...
| UpdatedAt | time.Time | 非空 | 当前时间 | 更新时间 |
| DeletedAt | gorm.DeletedAt | 可为空 | NULL | 软删除时间 |
| Template | string(100) | 可为空 | NULL | 模板名称 |
| TemplateURL | string(2048) | 可为空 | NULL | 模板 URL |
| TemplateID | string(200) | 可为空 | NULL | 模板 ID |
| MatcherName | string(255) | 可为空 | NULL | 匹配器名称 |
| Name | string(255) | 非空 | - | 漏洞名称 |
| Severity | int | 非空 | -1 | 严重等级 |
| Description | text | 可为空 | NULL | 漏洞描述 |
| Impact | text | 可为空 | NULL | 影响评估 |
| Remediation | text | 可为空 | NULL | 修复建议 |
| ExtractedResults | []string | 可为空 | [] | 提取结果 |
| CVSSMetrics | string(500) | 可为空 | NULL | CVSS 向量 |
| CVSSScore | float64 | 可为空 | 0 | CVSS 分数 |
| CurlCommand | string(4096) | 可为空 | NULL | 复现命令（可加密） |
...
| Request | text | 可为空 | NULL | 请求包 |
| Response | text | 可为空 | NULL | 响应包 |
...
| Command | text | 可为空 | NULL | 执行命令 |
| ReturnCode | int | 可为空 | 0 | 返回码 |
| Output | text | 可为空 | NULL | 命令输出 |
| Time | time.Time | 非空 | - | 执行时间 |
...
| YAMLConfiguration | text | 非空 | - | YAML 配置内容 |
...
| Description | text | 可为空 | NULL | 笔记内容 |
...
| ReportTemplate | text | 可为空 | NULL | 报告模板 |
...
| ExecutiveSummaryDescription | text | 可为空 | NULL | 执行摘要描述 |
