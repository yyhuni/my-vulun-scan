...
| DomainID | uint | 可为空，外键 | NULL | 所属域名ID |
| SubdomainID | uint | 可为空，外键 | NULL | 所属子域名ID |

**业务逻辑约束**:
- **灵活归属**: DomainID 和 SubdomainID 至少有一个非空
- **级联删除**: 删除域名或子域名时自动删除相关端点

**索引优化**:
- `URL` 字段建立索引，优化查询性能
- 复合索引 (DomainID, SubdomainID) 优化关联查询
...
| SubdomainID | uint | 可为空，外键 | NULL | 所属子域名 ID |
| EndPointID | uint | 可为空，外键 | NULL | 所属端点 ID |
| DomainID | uint | 可为空，外键 | NULL | 目标域名 ID |
...
**索引优化**:
- `Severity` 字段建立索引，优化按严重等级查询
- `CVE` 字段建立索引，优化 CVE 编号查询
- `Status` 字段建立索引，优化状态查询
- 复合索引 (DomainID, SubdomainID, EndPointID) 优化关联查询
- `DiscoveredDate` 字段建立索引，优化时间范围查询
