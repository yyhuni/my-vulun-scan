...
| CreatedAt | time.Time | 非空 | 当前时间 | 创建时间 |
| UpdatedAt | time.Time | 非空 | 当前时间 | 更新时间 |
| Name | string(255) | 唯一，非空，索引，**强制小写存储** (应用层转换 + CHECK约束) | - | 规范化域名名称（统一小写） |
| Description | varchar(1000) | 可为空 | NULL | 描述信息 |
...
| CreatedAt | time.Time | 非空 | 当前时间 | 创建时间 |
| UpdatedAt | time.Time | 非空 | 当前时间 | 更新时间 |
| Name | string(255) | 非空，索引，唯一，只读(创建后)，**强制小写存储** (应用层转换 + CHECK约束) | - | 子域名名称（统一小写，根子域名与 Domain 同名） |
...
- **小写规范化**: Name 字段在应用层统一转为小写存储，数据库层通过 `CHECK (name = LOWER(name))` 约束防止插入大写值
- **子域名归属验证**: 创建子域名时验证是否以 `.父域名` 结尾，防止将独立域名错误添加为子域名
...
- **冗余字段**: DomainID 从 Subdomain.DomainID 自动获取，用于优化查询性能
