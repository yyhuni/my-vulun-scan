...
-- 2) 插入子域名端点（每个子域名 5 个端点）
-- 为前10个子域名插入端点示例
INSERT INTO endpoints (id, url, method, status_code, title, content_length, domain_id, subdomain_id, created_at, updated_at) VALUES
-- Subdomain 1 (www.org1-example-01.com) 的端点
...
-- Subdomain 2 (api.org1-example-01.com) 的端点
...
-- Subdomain 3 (admin.org1-example-01.com) 的端点
...
-- Subdomain 4 (mail.org1-example-01.com) 的端点
...
DECLARE
    ...
    url TEXT;
BEGIN
    -- 为剩余域名生成端点 (domain_id 3-400, 每个域名15个端点)
    ...
    -- 为剩余子域名生成端点 (subdomain_id 5-4000, 每个子域名5个端点)
    FOR subdomain_id_var IN 5..4000 LOOP
        SELECT sd.name, sd.domain_id INTO subdomain_name, domain_id_var 
        FROM sub_domains sd WHERE sd.id = subdomain_id_var;
        ...
        FOR i IN 1..5 LOOP
            ...
            url := 'https://' || subdomain_name || path;
            title := 'Subdomain Endpoint ' || i || ' for ' || subdomain_name;
...
-- 计算总端点数：400个域名×15个端点 + 4000个子域名×5个端点 = 6000 + 20000 = 26000
