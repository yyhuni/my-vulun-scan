...
export function EditScheduledScanDialog({
  ...
  React.useEffect(() => {
    if (scheduledScan && open) {
      ...
      setDescription(scheduledScan.description || "")
      setEngineId(scheduledScan.engine)
      setSelectedTargetIds(scheduledScan.targetIds || [])
          ...
          <div className="grid gap-2">
              ...
              {targets.length === 0 ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {targets.map((target) => (
                    <Badge
                      key={target.id}
                      variant={selectedTargetIds.includes(target.id) ? "default" : "outline"}
            ...
            {selectedTargetIds.length > 0 && (
              <p className="text-xs text-muted-foreground">
                已选择 {selectedTargetIds.length} 个目标
              </p>
            )}
          </div>
