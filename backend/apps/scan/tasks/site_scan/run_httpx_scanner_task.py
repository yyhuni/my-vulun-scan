...
@task(
    name='run_httpx_scanner',
    retries=0,
    log_prints=True
)
def run_httpx_scanner_task(
    tool: str,
    target_file: str,
    result_dir: str,
    command: str,
    ...
    Args:
        ...
        target_file: 目标URL列表文件路径
        result_dir: 结果目录（已拼接好完整路径）
        command: 扫描命令模板（如：'httpx -l {target_file} -o {output_file} -json'）
        timeout: 命令执行超时时间（秒）
    ...
    Note:
        ...
        - 文件命名格式：{tool}_{timestamp}_{uuid4}.json
    """
    logger.info("开始运行站点扫描工具: %s - 目标文件: %s", tool, target_file)
    ...
    output_file = result_path / f"{file_prefix}.json"
