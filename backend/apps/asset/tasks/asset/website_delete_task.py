...
def hard_delete_website_task(website_id: int) -> Dict:
    """
    ...
    try:
        logger.info(f"🔵 开始删除网站 (ID: {website_id})")
        ...
        logger.info(
            f"✓ 删除完成 (ID: {website_id}) - 删除 {deleted_count:,} 条记录，耗时 {execution_time:.2f}s"
        )
        ...
        execution_time = time.time() - start_time
        logger.error(
            f"❌ 删除失败 (ID: {website_id}) - 错误: {e}, 耗时 {execution_time:.2f}s", 
            exc_info=True
        )
        raise
