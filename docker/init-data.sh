...
#
# 被以下脚本调用：
#   - install-dev.sh / install.sh（安装时）
#   - start-dev.sh / start.sh（启动时，可选）
#   - update-dev.sh / update.sh（更新时）
#
...
log_step() { echo -e "${BLUE}[INIT]${NC} $1"; }
...
if engine:
    if not engine.configuration or not engine.configuration.strip():
        ...
        print(f'已初始化默认引擎配置: {engine.name}')
    else:
        print(f'默认引擎已有配置，跳过')
else:
    # 创建默认引擎
