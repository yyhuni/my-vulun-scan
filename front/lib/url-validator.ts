...
export class UrlValidator {
   ...
   * @returns 验证结果
   */
  static validate(urlString: string): UrlValidationResult {
    // 1. 检查是否为空
    ...
    if (!validator.isURL(trimmedUrl, {
      ...
      return {
        isValid: false,
        error: 'URL 格式无效，必须是有效的 HTTP/HTTPS URL'
      }
    }
   ...
   * @returns 验证结果数组
   */
  static validateBatch(urls: string[]): Array<UrlValidationResult & { index: number; originalUrl: string }> {
